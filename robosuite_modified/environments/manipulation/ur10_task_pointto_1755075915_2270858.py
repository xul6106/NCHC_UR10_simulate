...
class DualTableTask(ManipulationEnv):
    ...
    def reward(self, action=None):
        ...
        distance_reward = np.exp(-5.0 * dist_to_drink)
        reward += distance_reward
        
        height_diff = abs(eef_pos[2] - target_drink_pos[2])
        height_reward = 0.5 * np.exp(-10.0 * height_diff)
        reward += height_reward
        
        horizontal_dist = np.linalg.norm(eef_pos[:2] - target_drink_pos[:2])
        horizontal_reward = 2.0 * np.exp(-8.0 * horizontal_dist)
    ...
    def _display_task_info(self):
        ...
        print(f"任務目標: 將機械手臂移動到 {drink_display_name} 前方")
    ...
    def _check_success(self):
        try:
            ...
            dist_to_drink = np.linalg.norm(eef_pos - target_drink_pos)
            success = dist_to_drink < self.success_threshold
            ...
            if success:
                ...
                print(f"\n🎉 SUCCESS! 機械手臂已成功到達 {drink_name} 前方!")
                print(f"距離: {dist_to_drink:.3f} 米 (閾值: {self.success_threshold:.3f} 米)")
